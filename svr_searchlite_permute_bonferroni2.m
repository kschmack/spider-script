...
%% SVR options
% data selection
conlist={{'visible spider','visible flower'}};%{'visible spider','visible flower'}
    ...
    for con=1:length(conlist)
        ...
        for preplab=1:length(preplabellist)
            ...
            for subind=1:length(sublist)
                a=label(subind);
                b=label;b(subind)=[];
                [bpreplabel,scalemax_matrix(subind),scalemin_matrix(subind)]=preprocess_data(b,preplabelstring);
                [apreplabel]=preprocess_data(a,preplabelstring,scalemax_matrix(subind),scalemin_matrix(subind));
                if subind==1
                    abpreplabel=[apreplabel;bpreplabel(subind:end)];
                elseif subind==length(sublist)
                    abpreplabel=[bpreplabel(1:subind-1);apreplabel];
                else
                    abpreplabel=[bpreplabel(1:subind-1);apreplabel;bpreplabel(subind:end)];
                end
                preplabel_matrix(:,subind)=abpreplabel;
            end
                ...
                for prepdat=1:length(prepdatalist)
                    ...
                    for ker=1:length(kernellist)
                        ...
                        %if ~exist(resfile,'file')
                            ...
                            if exist(fullfile(fileparts(resfile),'temp.mat'),'file')
                                load(fullfile(fileparts(resfile),'temp.mat'))
                                kstart=k;
                            ...
                            for k=1:length(XYZmm) % start voxel loop
                                ...
                                a(k)=sum(s);
                                 ...
                                 for sub=sublist
                                     ...
                                     testlabel=preplabel_matrix(ismember(sublist,sub),ismember(sublist,sub));
                                     traindata=prepimval_s(~ismember(sublist,sub),:);
                                     trainlabel=preplabel_matrix(~ismember(sublist,sub),ismember(sublist,sub));
                                 ...
                                 %if rem(k,100)==0
                                     save(fullfile(fileparts(resfile),'temp.mat'),'k','cr','mcrp','ncrp','pcrp')
                                 %end
                            end
