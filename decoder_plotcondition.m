...
voxellist=[250];
overwrite=1;
conlist={{'visible flower','visible spider'},{'invisible flower','invisible spider'}};%,};%,{'visible ','invisible '}};
preplist={'vector'};%,
...
feature_list={fullfile('..','data','spi_mri_0_0XX','unistat','loc_nonorm_nosmooth','spmT_0001.img')};
mask_list={...
    fullfile('..','data','spi_mri_0_0XX','mask','mwspi_mri_0_0XX_fusiformlatmidocctemp_gyrus_wfupick.nii'),
    fullfile('..','data','spi_mri_0_0XX','mask','sculp_sspi_mri_0_0XX.nii'),};
%fullfile('..','data','spi_mri_0_0XX','mask','loc_bilateral_sspi_mri_0_0XX.nii'),...
%     fullfile('..','data','spi_mri_0_0XX','mask','spi_mri_0_0XX_anatomic_l
%     ocp001.nii'),...
...
for f=1:length(feature_list)
    ...
    for m=1:length(mask_list)
        ...
        for s=1:length(spm_path_list)
            ...
            [ht p ci stat]=ttest(plotacccon,.50,.05,'right');
            for sa=1:size(plotacccon,2)
                [lp,hw(sa),stats]=signrank(plotacccon(:,sa),.5);
                tvalnon(1,sa)=stats.signedrank;
                pvalnon(1,sa)=lp./2;
            end
            ...
            % plot
            data=plotacccon*100;
            myfigure(.8,.8);%set(gcf,'Color','w');
            errorbar(mean(data),std(data)./sqrt(size(data,1)),' w.','MarkerSize',30,'LineWidth',2);
            hold on;
            plot([0 1 2 3],[50 50 50 50],'--w','LineWidth',2)
            hold on;
            ysig=mean(data)-std(data)./sqrt(size(data,1))-2;
            xsig=1:2;
            plot(xsig(hw==1),ysig(hw==1),' w*','MarkerSize',10);
            ylim([25 100]);
            xlim([.5 2.5]);
            ylabel('Decoding Accuracy (%)')
            h = my_xticklabels(gca,[1 2],{sprintf('visible SPIDER vs\nvisible FLOWER'),sprintf('invisible SPIDER\nvs invisible FLOWER')});
            axis square
            [a mstr]=fileparts(mask_path);
            
            %titel
            if strcmp(mstr,'loc_bilateral_sspi_mri_0_0XX');
                str1='LOC';
            elseif strcmp(mstr,'spi_mri_0_0XX_anatomic_locp001');
                str1='LOC';
            elseif strcmp(mstr,'mwspi_mri_0_0XX_fusiformlatmidocctemp_gyrus_wfupick');
                str1='LO & FUS';
            elseif strcmp(mstr,'sculp_sspi_mri_0_0XX');
                str1='SKULL';
            end
            str2=strrep(ftrstr,'spmT_0001','object');
            str2=strrep(str2,'ttest','informative');

%             if isempty(spm_path(strfind(spm_path,'norealign'):end))
%                 str2='(with realignment parameters)';
%             else str2='(without realignment parameters)';
%             end
            title(sprintf('%s %s',str1,str2),'FontSize',20)
            set(gcf,'InvertHardCopy','off','PaperPositionMode','auto');
            optname=fullfile('..','figures',sprintf('%s_%s_%s.jpg',mstr,spm_path(strfind(spm_path,'norealign'):end),ftrstr));
            print(optname,'-djpeg','-r600');
            close
            fid=fopen(strrep(optname,'jpg','txt'),'w');
            yl={sprintf('invisible ALL vs\nvisible ALL'),sprintf('visible SPIDER vs\nvisible FLOWER'),sprintf('invisible SPIDER\nvs invisible FLOWER')};
            for c=1:length(conlist)
                fprintf(fid,'%s\nmean accuracy=%2.2f%s\nstd=%2.2f%s\nt(19)=%2.2f\np=%2.4f\nsigned rank statistic(19)=%2.2f\np=%2.4f\n\n',yl{c},mean(data(:,c)),'%',std(data(:,c)),'%',stat.tstat(c),p(c),tvalnon(c),pvalnon(c));
            end
            fclose(fid);
