    ...
    for c=1:length(conlist)
        ...
        for p=1:length(preplist)
            ...
            for sub=1:length(sublist)
                ...
                resstr=sprintf('%s%s_vs_%s%s_crossset_%s_radius%02.0fmm_spi_mri_0_0%02.0f.mat',str1{1}(1:3),str1{2}(1:3),str2{1}(1:3),str2{2}(1:3),prep,radius,sublist(sub));
                    ...
                    for k=1:length(XYZmm) % start voxel loop
                        ...
                        for sets=1:2% start set loop
                            for run=unique(runlabel)' %start run loop
                                ...
                                testindex=ismember(runlabel,run)&ismember(label,[(sets-1)*2+1 (sets-1)*2+2]);
                                trainindex=~ismember(runlabel,run)&~ismember(label,[(sets-1)*2+1 (sets-1)*2+2]);
                                ...
                                % output                               
                                cr((sets-1)*8+run,k)=sum((testlabel==1)&(testlabel==predlabel));
                                fr((sets-1)*8+run,k)=sum((testlabel==1)&(testlabel~=predlabel));
                                cl((sets-1)*8+run,k)=sum((testlabel==2)&(testlabel==predlabel));
                                fl((sets-1)*8+run,k)=sum((testlabel==2)&(testlabel~=predlabel));
                            end
                        end
                    end
