...
voxellist=[50:50:500];
overwrite=1;
conlist={{'invisible flower','invisible spider'}};%,};%,{'visible ','invisible '}};{'visible ','invisible '},{'visible flower','visible spider'},
preplist={'vector'};%,
...
feature_list={'ttest',fullfile('..','data','spi_mri_0_0XX','unistat','loc_nonorm_nosmooth','spmT_0001.img')};
mask_list={fullfile('..','data','spi_mri_0_0XX','mask','mwspi_mri_0_0XX_fusiformlatmidocctemp_gyrus_wfupick.nii')};%,fullfile('..','data','spi_mri_0_0XX','mask','loc_bilateral_sspi_mri_0_0XX.nii')};

%    fullfile('..','data','spi_mri_0_0XX','mask','spi_mri_0_0XX_anatomic_lo
%    cp001.nii'),...

loops=length(spm_path_list)*length(conlist)*length(preplist)*length(sublist)*length(voxellist)*length(feature_list)*length(mask_list);
l=0;
hw=waitbar(0,'Decoder working...');
for s=1:length(spm_path_list)
    spm_path=spm_path_list{s};
    clear('ylab')
    
    for f=1:length(feature_list)
        feature_path=feature_list{f};
        for m=1:length(mask_list)
            mask_path=mask_list{m};
            for c=1:length(conlist)
                ...
                for p=1:length(preplist)
                    ...
                    for sub=1:length(sublist)
                        for vox=1:length(voxellist);
                            ...
                            if ~exist(resfile,'file');
                                if voxellist(vox)>250&&strcmp(mskstr,'loc_bilateral_sspi_mri_0_0XX')
                                    crossacc=nan;
                                else
                    ...
                    plotacc(f,m,:)=mean(accuracy);
                    e(f,m,:)=std(accuracy*100);
                    [h pval(f,m,:) ci stat]=ttest(accuracy,.5,0.05,'right');
                    tval(f,m,:)=stat.tstat;
                    for sa=1:size(accuracy,2)
                        if ~isnan(sum(accuracy(:,sa)))
                        [lp,h,stats]=signrank(accuracy(:,sa),.5);
                        tvalnon(f,m,sa)=stats.signedrank;
                        pvalnon(f,m,sa)=lp./2;
                        else pvalnon(f,m,sa)=nan;tvalnon(f,m,sa)=nan;end
                    end
                    str1=strrep(ftrstr,'spmT_0001','object');
                    str1=strrep(str1,'ttest','informative');
                    str2=strrep(mskstr,'loc_bilateral_sspi_mri_0_0XX','LOC');
                    str2=strrep(str2,'fusiformlatmidocctemp_gyrus_wfupick','LO & FUS');
                    ylab{f,m}=[str2 ' ' str1];
                end %prep list
            end %conditions loop
        end %feature path list
    end %mask list
    % % compare feature selection
    accim=reshape(plotacc,length(feature_list)*length(mask_list),length(voxellist)).*100;
    mm=accim;
    e=reshape(e,length(feature_list)*length(mask_list),length(voxellist));
    pval=reshape(pval,length(feature_list)*length(mask_list),length(voxellist));
    tval=reshape(tval,length(feature_list)*length(mask_list),length(voxellist));
    pvalnon=reshape(pvalnon,length(feature_list)*length(mask_list),length(voxellist));
    tvalnon=reshape(tvalnon,length(feature_list)*length(mask_list),length(voxellist));

    ylab=reshape(ylab,1,length(feature_list)*length(mask_list));
    f=myfigure(1,.4);%('Position',[20 20 scrsz(3)*.9 scrsz(4).*9],'Color','k');
    im=imagesc([accim;ones(1,size(accim,2))*60;ones(1,size(accim,2))*45]);
    colormap('jet');
    ...
    ylabel('Voxel Selection','Color','w')
    xlim([0.5 size(accim,2)+.5])
    ylim([0.5 length(ylab)+.5])
    title('Decoding Accuracy','Interpreter','none')
    [a b]=max(accim);
    [c d]=max(a);            
    line([-.5,.5;-.5,.5;-.5 -.5;.5 .5]+d,[-.5 -.5;.5 .5;-.5 .5;-.5 .5]+b(d),'LineWidth',3,'Color','k');
    colorbar('YTick',[45,50,55,60],'YTickLabel',{'45%','50%','55%','60%'},'Color','k','XColor','w','YColor','w','ZColor','w','FontName','Arial','FontSize',14)%print('Optimiz','-djpeg','-r200');
    set(gcf,'InvertHardCopy','off','PaperPositionMode','auto');
    cdata=get(im,'CData');
    adata=~isnan(cdata);
    set(im,'AlphaData',adata);
    if ~isempty(spm_path(strfind(spm_path,'norealign'):end))
        optname=fullfile('..','figures','featureselection_norealign.jpg');
    else optname=fullfile('..','figures','featureselection_realign.jpg');
    end
    print(optname,'-djpeg','-r600');
    fid=fopen(strrep(optname,'jpg','txt'),'w');
    fprintf(fid,'mean accuracy=%2.2f%s\nstd=%2.2f%s\nt(19)=%2.2f\np=%2.4f\nsigned rank statistic(19)=%2.2f\np=%2.4f',mm(b(d),d),'%',e(b(d),d),'%',tval(b(d),d),pval(b(d),d),tvalnon(b(d),d),pvalnon(b(d),d));     
    fclose(fid);
    close;
    clear('e','mm','accim','plotacc','pval','tval','pvalnon','tvalnon');
