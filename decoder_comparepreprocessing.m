...
voxellist=[100:50:1000];
overwrite=1;
conlist={{'invisible flower','invisible spider'}};%,};%,{'visible ','invisible '}};
...
mask_list={fullfile('..','data','spi_mri_0_0XX','mask','mwspi_mri_0_0XX_fusiformlatmidocctemp_gyrus_wfupick.nii')};
        ...
        for s=1:length(spm_path_list)
            ...
            for c=1:length(conlist)
                ...
                for p=1:length(preplist)
                    ...
                    mm(p,:)=mean(accuracy*100);
                    e(p,:)=std(accuracy*100);
                    [h pval(p,:) ci stat]=ttest(accuracy,.5,0.05,'right');
                    tval(p,:)=stat.tstat;
                    for sa=1:size(accuracy,2)
                        [lp,h,stats]=signrank(accuracy(:,sa),.5);
                        tvalnon(p,sa)=stats.signedrank;
                        pvalnon(p,sa)=lp./2;
                    end
            ...
            imagesc([mm;ones(1,size(mm,2))*60;ones(1,size(mm,2))*45]);
            colormap('jet');
            ...
            if ~isempty(spm_path(strfind(spm_path,'norealign'):end))
                optname=fullfile('..','figures','preprocessselection_norealign.jpg');
            else     optname=fullfile('..','figures','preprocessselection_realign.jpg');
                
            end
            print(optname,'-djpeg','-r600');
            fid=fopen(strrep(optname,'jpg','txt'),'w');
            fprintf(fid,'mean accuracy=%2.2f%s\nstd=%2.2f%s\nt(19)=%2.2f\np=%2.4f\nsigned rank statistic(19)=%2.2f\np=%2.4f\n',mm(b(d),d),'%',e(b(d),d),'%',tval(b(d),d),pval(b(d),d),tvalnon(b(d),d),pvalnon(b(d),d));
            fclose(fid);
            close;
